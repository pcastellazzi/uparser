"""
Minimalistic parser combinator library using a functional approach, sum types
...
class Failure[F]:
    ...
    Parameters:
        * index: index on the input where the error was found.
        * error: Error found.
    ...
    Examples:
        ...
        Failure(index=1, error=['some', 'other', 'error'])
...
class Success[S]:
    ...
    Parameters:
        * index: index on the input where the value was found.
        * value: Value parsed.
    ...
    Examples:
        ...
        Success(index=0, value='some value')

        # A list of strings.
        >>> p.Success(1, ["some", "other", "value"])
        Success(index=1, value=['some', 'other', 'value'])
...
def parser_hook[F, S](
    ...
    This implementation uses `functools.wraps` and `functools.cache` to enable
    a nice output when debugging a parser and a caching mechanism to avoid
    recalculations.

    Parameters:
        caller: The parser generator function.
    ...
    Examples:
        ...
        # disable hooks using an identity function
...
def regex(expression: "str | Pattern[str]") -> Parser[str, str]:
    ...
    Examples:
        ...
        >>> parser2 = p.regex(re.compile(r"\d+ # comment", re.VERBOSE))
        >>> parser2(0, "letters")
...
def map_error[F, F1, S](
    element: Parser[F, S], fn: "Callable[[F], F1]"
) -> Parser[F1, S]:
    """
    ...
    return parser_hook(map_error)(
        map(
            element,
            lambda state: Failure(state.index, fn(state.error))
            if isinstance(state, Failure)
            else state,
        )
    )
...
def map_value[F, S, S1](
    element: Parser[F, S], fn: "Callable[[S], S1]"
) -> Parser[F, S1]:
    """
    ...
    return parser_hook(map_value)(
        map(
            element,
            lambda state: Success(state.index, fn(state.value))
            if isinstance(state, Success)
            else state,
        )
    )
